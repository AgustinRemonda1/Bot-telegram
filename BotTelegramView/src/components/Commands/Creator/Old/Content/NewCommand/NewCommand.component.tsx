...
const NewCommand = (props: NewCommandProps) => {
  ...
  const commandListToOptions = useMemo<OptionInterface[]>(() => {
    return botCommandList
      ? botCommandList.map((command) => ({
          id: Number(command.bot_command_id),
          name: command.name
        }))
      : [];
  }, [botCommandList]);
  ...
  const addNestedCommand = useCallback(() => {
    if (commandToAdd && botCommandList) {
      const nestedCommand = botCommandList.find(
        (command) => command.bot_command_id === Number(commandToAdd)
      );
  ...
  const commandCharge = useCallback(() => {
    if (editMode && botCommandSelected) {
      ...
      botResponses &&
        botResponses.description &&
        setDescription(botResponses.description);
      setUserType(botCommandSelected.user_type_id);
      setCommandType(botCommandSelected.command_type_id);
      botResponses &&
        botResponses.response &&
        setResponse(botResponses.response);
      botResponsesFiles &&
        botResponsesFiles.filename &&
        setFileName(botResponsesFiles.filename);
      botResponsesFiles &&
        botResponsesFiles.url &&
        setUrl(botResponsesFiles.url);
      botResponses &&
        botResponses.description &&
        setButtonList(botResponses.description);
      botResponses &&
        botResponses.description &&
        setCoordinates(botResponses.description);
  ...
  const handleDeleteNestedCommand = (id: number) => {
    const commandsWithoutNestedCommand = commandsAdded.filter(
      (command) => command.bot_command_id !== id
    );
