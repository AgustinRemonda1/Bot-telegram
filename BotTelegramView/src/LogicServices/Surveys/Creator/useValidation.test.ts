...
describe('useValidation', () => {
  it('has every fields', async () => {
    const { result } = renderHook(() =>
      useValidation({ survey: surveyDataMock[0] })
    );
  ...
  it('missing name field', async () => {
    const { result } = renderHook(() =>
      useValidation({ survey: { ...surveyDataMock[0], name: '' } })
    );
  ...
  it('missing description field', async () => {
    const { result } = renderHook(() =>
      useValidation({ survey: { ...surveyDataMock[0], description: '' } })
    );
  ...
  it('missing userTypeId field', async () => {
    const { result } = renderHook(() =>
      useValidation({ survey: { ...surveyDataMock[0], userTypeId: 0 } })
    );
